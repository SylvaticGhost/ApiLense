...
export class DependencyContainer implements DependencyProvider {
  ...
  register<TInstance>(
    func: (container: DependencyContainer) => TInstance,
    name: string,
  ) {
  ...
  registerDbRepo<TRepository>(
    func: (prismaClient: PrismaClient) => TRepository,
    name: string,
  ) {
  ...
  async registerAsync<TInstance>(
    func: (container: DependencyContainer) => Promise<TInstance>,
    name: string,
  ): Promise<void> {
