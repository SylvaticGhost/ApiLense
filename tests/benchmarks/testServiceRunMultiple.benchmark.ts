...
const templateFillingService = container.resolve<TemplateFillingService>(
...
if (!loadResult.isSuccess())
  throw new Error('Schema load failed for benchmark setup');
...
if (!templateResult.isSuccess())
  throw new Error('Template creation failed in benchmark setup');
...
if (templateFilling) {
  ...
  const tfRepo = container.resolve<TemplateFillingRepository>(
    'TemplateFillingRepository',
  );
  const endpointResult = await endpointService.getEndpointByRef(
    schemaId!,
    templateEndpointName,
  );
...
for (const size of sizes) {
  for (const concurrency of concurrencies) {
    Deno.bench({
      ...
      fn: async () => {
        const res = await testService.runEndpoint({
          schema: schemaId!,
        ...
        if (!res.isSuccess())
          throw new Error(`runEndpoint POST returned ${res.errorMessage}`);
      },
    });
  }
}
...
let getTemplateFillingName = '';
{
  ...
  if (!endpointResult.isSuccess())
    throw new Error(`Endpoint ${getEndpoint} not found in schema ${schemaId}`);
  const endpoint = endpointResult.castValueStrict<any>();
  const created = TemplateFilling.create(
    getTemplateName,
    schemaId!,
    endpoint,
    endpoint.template,
    ...
    Deno.bench({
      ...
      fn: async () => {
        const res = await testService.runEndpoint({
          ...
          template: getTemplateFillingName,
          numberOfRequests: size,
          concurrency,
          delayMs,
          mode: 'multiple',
        ...
        if (!res.isSuccess())
          throw new Error(`runEndpoint GET returned ${res.errorMessage}`);
      },
    });
  }
}
